...
function getVehiclesByClassification($classificationName){
    $db = phpmotorsConnect();
    $sql = 'SELECT inventory.invId, inventory.invMake, inventory.invModel, inventory.invPrice, images.imgPath AS invThumbnail 
...
function getVehicleDetails($invId) {
    $db = phpmotorsConnect();
    $sql = 'SELECT inv.invId, inv.invMake, inv.invModel, inv.invDescription, inv.invStock, inv.invColor, inv.invPrice, img.imgPath AS invImage 
...
function getVehicleThumbnails($invId) {
    $db = phpmotorsConnect();
    $sql = 'SELECT inv.invId, inv.invMake, inv.invModel, img.imgPath AS invThumbnail 
            FROM inventory AS inv 
            LEFT JOIN images AS img ON inv.invId = img.invId AND img.imgPath LIKE "%-tn%"
            WHERE inv.invId = :invId';
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
    $stmt->execute();
    $invThum = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
    return $invThum;
}
