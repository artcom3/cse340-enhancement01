...
function buildInvItemDisplay($vehicle) {
    ...
    $dv .= "<p><b>Price: $".number_format($vehicle['invPrice'], 2)."</b></p>";
    $dv .= '</div>';
    $dv .= '<div id="veh-details">';
...
function buildThumbnailsDisplay($thumbnails) {
    $dt = '<div id="veh-thumbnails">';
    $dt .= '<h2>Thumbnails</h2>';
    $dt .= '<ul>';
    foreach ($thumbnails as $thumbnail) {
        $dt .= '<li>';
        $dt .= "<img src='$thumbnail[invThumbnail]' alt='$thumbnail[invMake] $thumbnail[invModel] thumbnail'>";
        $dt .= '</li>';
    }
    $dt .= '</ul>';
    $dt .= '</div>';
